...
import { saveNewsletterConfigAction } from "@/actions/newsletter-actions"
...
export function NewsletterClient({ config }: { config: NewsletterConfig }) {
  const [enabled, setEnabled] = useState(config.enabled)
  const [title, setTitle] = useState(config.title)
  const [description, setDescription] = useState(config.description)
  const [ctaLabel, setCtaLabel] = useState(config.cta_label)
  const [successMessage, setSuccessMessage] = useState(config.success_message)
  const [placement, setPlacement] = useState<Placement>(config.placement as Placement)
  const [trigger, setTrigger] = useState<Trigger>(config.trigger_type as Trigger)
  const [triggerValue, setTriggerValue] = useState(config.trigger_value.toString())
  const [askName, setAskName] = useState(config.ask_name)
  const [privacyConsent, setPrivacyConsent] = useState(config.privacy_consent)
  const [incentive, setIncentive] = useState(config.incentive)
  const [incentiveText, setIncentiveText] = useState(config.incentive_text)
  ...
  const handleSave = () => {
    startTransition(async () => {
      try {
        await saveNewsletterConfigAction({
          enabled,
          title,
          description,
          cta_label: ctaLabel,
          ...
          trigger_value: parseInt(triggerValue) || 0,
          ask_name: askName,
                    ...
                    <div className="flex items-start gap-3">
                      ...
                      <div>
                        ...
                        <p className="mt-0.5 text-xs text-muted-foreground">
                          Todos os leads ficam disponíveis em /app/leads
                        </p>
                      </div>
                    </div>
                  </div>
