...
import { Calendar, Edit2, Eye, FileText, MoreHorizontal, Search, Trash2, X } from "lucide-react"
...
export function ArticlesTable({
  ...
  const visible = useMemo(
    () => articles.filter((article) => article.status !== "queued" && article.status !== "generating"),
    [articles],
  )
  ...
  const counts = useMemo(
    () => ({
      all: visible.length,
      draft: visible.filter((article) => article.status === "draft").length,
      review: visible.filter((article) => article.status === "review").length,
      scheduled: visible.filter((article) => article.status === "scheduled").length,
      published: visible.filter((article) => article.status === "published").length,
    }),
    [visible],
  )
  ...
  const filtered = useMemo(() => {
    let list = visible
    if (filter !== "all") {
      list = list.filter((article) => article.status === filter)
    }
    if (search.trim()) {
  ...
  const allFilteredSelected =
    filtered.length > 0 && filtered.every((article) => selected.has(article.id))
        ...
        <Card className="flex flex-col items-center justify-center gap-3 border-dashed py-12 text-center">
          ...
          <div>
            <p className="font-medium text-foreground">
              {visible.length === 0 ? emptyLabel : "Nenhum resultado para os filtros atuais"}
            </p>
                ...
                return (
                  <TableRow
                    ...
                    className={cn(
                      ...
                      isSelected && "bg-primary/5 hover:bg-primary/5 data-[state=selected]:bg-primary/5",
                    )}
                  ...
                  >
                    ...
                    <TableCell className="max-w-[480px]">
                      <span className="block truncate text-foreground" title={article.title}>
                        {article.title}
                      </span>
                    </TableCell>
                    <TableCell>
                    ...
                    <TableCell className="text-right" onClick={(event) => event.stopPropagation()}>
                      <HoverCard>
                        ...
                        <HoverCardContent align="end" className="w-auto min-w-[180px] p-3">
                          <p className="mb-1.5 text-[11px] font-medium uppercase tracking-wide text-muted-foreground">
                            Palavras-chave
                          </p>
                          <div className="flex flex-wrap gap-1">
                            {article.keywords.length === 0 ? (
                              <span className="text-xs text-muted-foreground">Sem keywords vinculadas.</span>
                            ) : (
