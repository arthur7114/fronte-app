...
> = {
  ...
  conversao: { label: "Conversão", icon: Zap },
}
...
export function StrategyPreview({ draft, onSubmit, canSubmit, isEditing = false, isPending = false }: Props) {
  ...
  const modeLabel = 
    draft.operation_mode === "automatic" ? "Piloto Automático" :
    draft.operation_mode === "assisted" ? "Assistido" : "Manual"
  const ModeIcon =
    draft.operation_mode === "automatic" ? Bot :
    draft.operation_mode === "assisted" ? Cpu : Settings
  ...
  return (
    <div className="space-y-4">
      ...
      {/* CTA */}
      <Button className="w-full gap-2" size="lg" disabled={!canSubmit || isPending} onClick={onSubmit}>
        <Sparkles className="h-4 w-4" />
        {isPending ? "Processando..." : isEditing ? "Salvar configurações" : "Criar estratégia"}
      </Button>
      {!isEditing && (
