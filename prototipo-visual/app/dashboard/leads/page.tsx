...
export default function LeadsPage() {
  ...
  }, [leads, search, originFilter, statusFilter])

  const stats = useMemo(() => {
    const total = leads.length
    const novos = leads.filter((l) => l.status === "novo").length
    const engajados = leads.filter((l) => l.status === "engajado").length
  ...
  const originBreakdown = useMemo(() => {
    const origins: LeadOrigin[] = ["inline", "popup", "footer", "cta"]
    return origins
      .map((origin) => ({
        origin,
        label: ORIGIN_LABELS[origin],
        count: leads.filter((l) => l.origin === origin).length,
      }))
        ...
        <Card className="lg:col-span-2">
          <CardContent className="p-6">
            <div className="mb-4 flex items-center justify-between">
              ...
              <Badge
                variant="secondary"
                className="bg-emerald-100 text-emerald-700 hover:bg-emerald-100 border-0 gap-1"
              >
      ...
      <td className="px-3 py-3">
        <DropdownMenu>
          ...
          <DropdownMenuContent align="end" className="w-48">
            <DropdownMenuItem
              onClick={() => updateLeadStatus(lead.id, "engajado")}
